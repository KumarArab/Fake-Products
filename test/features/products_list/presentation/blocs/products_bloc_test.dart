...
void main() {
  ...
  group("Products Bloc Test", () {
    ...
    blocTest<ProductsBloc, ProductsState>(
      ...
      build: () =>
          ProductsBloc(productsRepo: mockIProductsRepo, eventBus: mockEventBus),
      setUp: () {
        when(() => mockIProductsRepo.getProductsList()).thenAnswer(
          (_) async => right(ProductModel.listFromJson(mockResponse)),
        );
      },
      act: (bloc) => bloc.add(FetchProductsEvent()),
      expect: () {
        final products = ProductModel.listFromJson(mockResponse);
        return [
          ProductsState.inital().copyWith(isloading: true),
          ProductsState.inital().copyWith(
            isloading: false,
            products: products,
            errorMessage: null,
          ),
